...
exports.COLOUR_MAP_DEFAULT = COLOUR_MAP_DEFAULT;
exports.COLOUR_MAP_MONOCHROME = COLOUR_MAP_MONOCHROME;
exports.COLOUR_MAP_INVERSE_MONOCHROME = COLOUR_MAP_INVERSE_MONOCHROME;

exports.COLOUR_MAP_NAMES = ['Default', 'Monochrome', 'Inverse monochrome'];

exports.create = (colourMapIndex) => {

    const colourMap = colourMaps[colourMapIndex];

    const numberOfColours = colourMap.length;

    const colourTableBuffer = new ArrayBuffer(numberOfColours * constants.BYTES_IN_UINT32);

    const colourTable = new Uint32Array(colourTableBuffer);

    for (let i = 0; i < numberOfColours; i += 1) {

        colourTable[i] = (0xFF << 24) | (colourMap[i][2] << 16) | (colourMap[i][1] << 8) | (colourMap[i][0] << 0);
